...
export async function checkStatus(response) {
  ...
  // Unauthorized (400)
  if (response.status === httpStatus.BAD_REQUEST) { // eslint-disable-line functional/no-conditional-statements
  ...
  // Forbidden (403)
  if (response.status === httpStatus.FORBIDDEN) { // eslint-disable-line functional/no-conditional-statements
  ...
  if (response.status === httpStatus.SERVICE_UNAVAILABLE) { // eslint-disable-line functional/no-conditional-statements
